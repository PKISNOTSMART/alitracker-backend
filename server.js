...
const db = require('./db');
const { scrapeProduct, initialize, resetSession } = require('./scraper');
...
// ── GLOBAL ERROR HANDLING (IMPORTANT FOR RENDER DEBUGGING) ──
  ...
  try {
    ...
    if (data.price) {
      db.prepare('INSERT INTO price_history (product_id, price) VALUES (?, ?)')
        .run(result.lastInsertRowid, data.price);
    }
...
app.post('/api/products/:id/refresh', async (req, res) => {
  ...
  try {
    ...
    const updated = db.prepare('SELECT * FROM products WHERE id = ?')
      .get(req.params.id);
...
const startServer = async () => {
  try {
    ...
    app.listen(PORT, "0.0.0.0", () => {
      log.success(`AliTracker API running on port ${PORT}`);
    });
